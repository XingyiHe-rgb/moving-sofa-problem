...
int main( ) {
    ...
    n2 = 0;
    nc2 = 0;
    double S, a, q, c, m, Area, (*XX)[n_area], h, hmin, hmax, alpha1, alpha2, RR, R, ds, ex, pi, K1[n], K2[n], K3[n], K4[n], (*Y1)[n], (*Y2)[n], (*Y3)[n], (*Y4)[n], (*x)[n],
            (*y)[n], (*xx)[n], (*yy)[n], (*Dx)[n], (*Dy)[n], *H1, *H2, H11, H22, *DH1, *DH2, xr1[2][4], xr2[2][4], xr3[2][4], xr4[2][4], xr5[2][4], xr6[2][4],
            xm1, xM1, xm2, xM2, (*Banana)[O1], *A, l1, l2, L1, L2, L3, L4, L5, L6, b1, b2, b3, b4, b5, b6, Fx[n], Fy[n], Fxx[n], Fyy[n],
            FH1, FH2, FH11, FH22, ds1, t;
    /*
        S: 沙发夹角
        n: 刚架个数
        n1: 时间
...
/*
    ...
    Initial_data.open("D-data-1pi3-100.txt", ios::in);
    ...
    for (int i = 0; i < nn; i++) {
        ...
        Initial_data >> H1[0] >> H2[0] >> n1;
    }
    x_Initial[0] = -0.5; y_Initial[0] = y_Initial[1]-(x_Initial[1]-(-0.5))*(y_Initial[1]-y_Initial[2])/(x_Initial[1]-x_Initial[2]);
    x_Initial[n_Initial+1] = -x_Initial[0]; y_Initial[n_Initial+1] = y_Initial[0];

    for (int i = 0; i < n_Initial+2; i++) s_Initial[i] = (pi-S)/2+i*S/(n_Initial+1);

    for(int i = 0; i < n; i++) {
        for (int j = 0; j < n_Initial+2; j++) {
            if (((pi-S)/2+(i+1)*S/(n+1) >= s_Initial[j])&&((pi-S)/2+(i+1)*S/(n+1) < s_Initial[j+1])) {
                x[0][i] = x_Initial[j] + (x_Initial[j+1]-x_Initial[j])*((pi-S)/2+(i+1)*S/(n+1)-s_Initial[j])/(s_Initial[j+1]-s_Initial[j]);
                y[0][i] = y_Initial[j] + (y_Initial[j+1]-y_Initial[j])*((pi-S)/2+(i+1)*S/(n+1)-s_Initial[j])/(s_Initial[j+1]-s_Initial[j]);
            }
        }
        Dx[0][i] = 0; Dy[0][i] = 0;
    }
...
*/
    ...
    for(int i = 1; i <= n; i++) {
        K1[i-1] = tan(pi/2-S+i*S/(n+1));
        K2[i-1] = K1[i-1];
        K3[i-1] = tan(pi/2+i*S/(n+1));
        K4[i-1] = K3[i-1];
    } //生成斜率
    ...
    while(RR >= ex) {
        ...
        for(int k = 0; k < n; k++) {
            ...
            Y2[n1][k] = y[n1][k]-x[n1][k]*K2[k]+(sin((pi-S)/2+(k+1)*S/(n+1))-K2[k]*cos((pi-S)/2+(k+1)*S/(n+1)))/sin(S/2);
            Y3[n1][k] = y[n1][k]-x[n1][k]*K3[k]+(sin((pi-S)/2+(k+1)*S/(n+1))-K3[k]*cos((pi-S)/2+(k+1)*S/(n+1)))/sin(S/2);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////第一根
                ...
                for (int k2 = 0; k2 < n; k2++) {
                    ...
                    if (K2[k1] > 0) {
                        if (xM1 > x[n1][k1] + (cos((pi-S)/2+(k1+1)*S/(n+1))) / sin(S / 2)) {
                            xM1 = x[n1][k1] + (cos((pi-S)/2+(k1+1)*S/(n+1))) / sin(S / 2);
                        }
                        if (xM2 > x[n1][k1] + (cos((pi-S)/2+(k1+1)*S/(n+1))) / sin(S / 2)) {
                            xM2 = x[n1][k1] + (cos((pi-S)/2+(k1+1)*S/(n+1))) / sin(S / 2);
                        }
                    }
                    if (K2[k1] < 0) {
                        if (xm1 < x[n1][k1] + (cos((pi-S)/2+(k1+1)*S/(n+1))) / sin(S / 2)) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////第四根
                ...
                Fx[k1] = q*(b1 - b2 + b3 - b4)*cos(S/2)*cos(-S/2+(k1+1)*S/(n+1))
                     -q*(- b1 + b2 + b3 - b4)*sin(S/2)*sin(-S/2+(k1+1)*S/(n+1));
                Fy[k1] = q*(b1 - b2 + b3 - b4)*cos(S/2)*sin(-S/2+(k1+1)*S/(n+1))
                     +q*(- b1 + b2 + b3 - b4)*sin(S/2)*cos(-S/2+(k1+1)*S/(n+1));
            ...
            for (int k2 = 0; k2 < n; k2++) {
                ...
                if ((0-K1[k2])*K1[k2] > 0) {
                    xr5[0][0] = 0;
                    xr5[1][0] = (Y1[n1][k2] - H1[n1]) / (0 - K1[k2]);
                } else {
                    xr5[0][0] = 1;
                    xr5[1][0] = (Y1[n1][k2] - H1[n1]) / (0 - K1[k2]);
                }
                ...
                if ((0-K2[k2])*K2[k2] > 0) {
                    xr5[0][1] = 1;
                    xr5[1][1] = (Y2[n1][k2] - H1[n1]) / (0 - K2[k2]);
                } else {
                    xr5[0][1] = 0;
                    xr5[1][1] = (Y2[n1][k2] - H1[n1]) / (0 - K2[k2]);
                }
                ...
                if ((0-K3[k2])*K3[k2] > 0) {
                    xr5[0][2] = 0;
                    xr5[1][2] = (Y3[n1][k2] - H1[n1]) / (0 - K3[k2]);
                } else {
                    xr5[0][2] = 1;
                    xr5[1][2] = (Y3[n1][k2] - H1[n1]) / (0 - K3[k2]);
                }
                ...
                if ((0-K4[k2])*K4[k2] > 0) {
                    xr5[0][3] = 1;
                    xr5[1][3] = (Y4[n1][k2] - H1[n1]) / (0 - K4[k2]);
                } else {
                    xr5[0][3] = 0;
                    xr5[1][3] = (Y4[n1][k2] - H1[n1]) / (0 - K4[k2]);
                } //计算相对四种边的单向范围
            ...
            if (A[0] == 0) L5 = 0;
                ...
                L5 = 0;
                for (int i = 1; i <= lround(A[0]); i++) L5 = L5 + A[2 * i] - A[2 * i - 1];
            }
            b5 = L5; //计算内部边长
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////直线架下面一根
            ...
            for (int k2 = 0; k2 < n; k2++) {
                ...
                if ((0-K1[k2])*K1[k2] > 0) {
                    xr6[0][0] = 0;
                    xr6[1][0] = (Y1[n1][k2] - H2[n1]) / (0 - K1[k2]);
                } else {
                    xr6[0][0] = 1;
                    xr6[1][0] = (Y1[n1][k2] - H2[n1]) / (0 - K1[k2]);
                }
                ...
                if ((0-K2[k2])*K2[k2] > 0) {
                    xr6[0][1] = 1;
                    xr6[1][1] = (Y2[n1][k2] - H2[n1]) / (0 - K2[k2]);
                } else {
                    xr6[0][1] = 0;
                    xr6[1][1] = (Y2[n1][k2] - H2[n1]) / (0 - K2[k2]);
                }
                ...
                if ((0-K3[k2])*K3[k2] > 0) {
                    xr6[0][2] = 0;
                    xr6[1][2] = (Y3[n1][k2] - H2[n1]) / (0 - K3[k2]);
                } else {
                    xr6[0][2] = 1;
                    xr6[1][2] = (Y3[n1][k2] - H2[n1]) / (0 - K3[k2]);
                }
                ...
                if ((0-K4[k2])*K4[k2] > 0) {
                    xr6[0][3] = 1;
                    xr6[1][3] = (Y4[n1][k2] - H2[n1]) / (0 - K4[k2]);
                } else {
                    xr6[0][3] = 0;
                    xr6[1][3] = (Y4[n1][k2] - H2[n1]) / (0 - K4[k2]);
                } //计算相对四种边的单向范围
                ...
                for (int i = 0; i < 3; i++) {
                    if ((xm1 < xr6[1][i]) && (xr6[0][i] == 0)) { xm1 = xr6[1][i]; }
                    if ((xM1 > xr6[1][i]) && (xr6[0][i] == 1)) { xM1 = xr6[1][i]; }
                    if ((xm2 < xr6[1][i + 1]) && (xr6[0][i + 1] == 0)) { xm2 = xr6[1][i + 1]; }
                    if ((xM2 > xr6[1][i + 1]) && (xr6[0][i + 1] == 1)) { xM2 = xr6[1][i + 1]; }
                }
            ...
            if (A[0] == 0) L6 = 0;
                ...
                L6 = 0;
                for (int i = 1; i <= lround(A[0]); i++) L6 = L6 + A[2 * i] - A[2 * i - 1];
            }
            b6 = L6; //计算内部边长
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////直线架上面一根
            ...
            if (ds1 < abs(H11 - H1[0]) + abs(H22 - H2[0]))
                ds1 = abs(H11 - H1[0]) + abs(H22 - H2[0]);
    ...
    if(1 == 1) {
        ...
        for (int ii = 0; ii <= n_area; ii++) {
            for (int k2 = 0; k2 < n; k2++) {
                ...
                if ((0 - K1[k2]) * K1[k2] > 0) {
                    xr5[0][0] = 0;
                    xr5[1][0] = (Y1[n1][k2] - H1[n1] - ii * (H2[n1]-H1[n1]) / n_area) / (0 - K1[k2]);
                } else {
                    xr5[0][0] = 1;
                    xr5[1][0] = (Y1[n1][k2] - H1[n1] - ii * (H2[n1]-H1[n1]) / n_area) / (0 - K1[k2]);
                }
                ...
                if ((0 - K2[k2]) * K2[k2] > 0) {
                    xr5[0][1] = 1;
                    xr5[1][1] = (Y2[n1][k2] - H1[n1] - ii * (H2[n1]-H1[n1]) / n_area) / (0 - K2[k2]);
                } else {
                    xr5[0][1] = 0;
                    xr5[1][1] = (Y2[n1][k2] - H1[n1] - ii * (H2[n1]-H1[n1]) / n_area) / (0 - K2[k2]);
                }
                ...
                if ((0 - K3[k2]) * K3[k2] > 0) {
                    xr5[0][2] = 0;
                    xr5[1][2] = (Y3[n1][k2] - H1[n1] - ii * (H2[n1]-H1[n1]) / n_area) / (0 - K3[k2]);
                } else {
                    xr5[0][2] = 1;
                    xr5[1][2] = (Y3[n1][k2] - H1[n1] - ii * (H2[n1]-H1[n1]) / n_area) / (0 - K3[k2]);
                }
                ...
                if ((0 - K4[k2]) * K4[k2] > 0) {
                    xr5[0][3] = 1;
                    xr5[1][3] = (Y4[n1][k2] - H1[n1] - ii * (H2[n1]-H1[n1]) / n_area) / (0 - K4[k2]);
                } else {
                    xr5[0][3] = 0;
                    xr5[1][3] = (Y4[n1][k2] - H1[n1] - ii * (H2[n1]-H1[n1]) / n_area) / (0 - K4[k2]);
                } //计算相对四种边的单向范围
            ...
            for (int i = 0; i < n/2; i++) {
                if ((H1[n1] + ii * (H2[n1]-H1[n1]) / n_area <= y[n1][i])&&(H1[n1] + ii * (H2[n1]-H1[n1]) / n_area > y[n1][i+1])) {

                    a = x[n1][i]+(x[n1][i+1]-x[n1][i])*((H1[n1] + ii * (H2[n1]-H1[n1]) / n_area)-y[n1][i])/(y[n1][i+1]-y[n1][i]);
                ...
                if ((H1[n1] + ii * (H2[n1]-H1[n1]) / n_area > y[n1][0]) && (H1[n1] + ii * (H2[n1]-H1[n1]) / n_area <= y[n1][0]-(x[n1][0]+0.5)*(y[n1][0]-y[n1][1])/(x[n1][0]-x[n1][1]))) {

                    a = y[n1][0]-(x[n1][0]+0.5)*(y[n1][0]-y[n1][1])/(x[n1][0]-x[n1][1]);

                    a = -0.5+(x[n1][0]-(-0.5))*((H1[n1] + ii * (H2[n1]-H1[n1]) / n_area)-a)/(y[n1][0]-a);
            ...
            if (ii != 0) Area += (l1 + l2)*(H2[n1]-H1[n1])/ (2 * n_area);
            l1 = l2;

            if(ii-n2==1000) {
                n2 = ii;
                cout << ii*1.0/n_area << endl;
            }
    ...
    ofstream out1("D-data-1pi3-100.txt");
    ...
    out1 << setw(20) << H1[n1] << setw(20) << H2[n1]<< setw(20) << 0 << "\n";
